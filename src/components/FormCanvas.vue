<template>
  <v-card class="form-canvas" elevation="2">
    ...
    <v-card-text>
      ...
      <div
        class="drop-zone"
        :class="{ 'drag-over': isDragOver, 'empty': elements.length === 0, 'grid-layout': elements.length > 0 }"
        @dragover.prevent="onDragOver"
        @dragleave="onDragLeave"
        @drop="onDrop"
      >
        <div v-if="elements.length === 0" class="empty-state">
          <v-icon icon="mdi-drag" size="48" color="grey-lighten-2" />
...
const elementRows = computed(() => {
  ...
  props.elements.forEach((element) => {
    ...
    if (element.newRow || currentRowWidth + elementWidth > 12 || (currentRow.length === 0 && rows.length === 0)) {
  ...
  if (event.dataTransfer) {
    try {
      ...
      if (data.type === 'form-element' && data.template) {
        ...
        emit('move-element', data.elementId, Math.min(newPosition, props.elements.length - 1));
      }
    } catch (error) {
